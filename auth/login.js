document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    const auth0Client = await createAuth0Client({
      ...
      authorizationParams: {
        redirect_uri: "https://jumatberkah.vercel.app/auth/callback",
      },
    });

    // Handle callback jika ada code di URL
    if (window.location.search.includes("code=")) {
      try {
        // Handle redirect callback
        await auth0Client.handleRedirectCallback();

        // Ambil token dan user info
        ...
        const response = await fetch(
          "https://backend-berkah.onrender.com/test-auth",
          {
            headers: {
        ...
        if (!response.ok) {
          throw new Error("Gagal verifikasi dengan backend");
        }

        const data = await response.json();

        // Simpan role dan userId dari backend
