document.addEventListener("DOMContentLoaded", () => {
  ...
  // Function to fetch masjid details based on user preferences or location
  async function fetchMasjidDetails(userEmail) {
    try {
      const response = await fetch(
        `https://api.example.com/masjid-details?email=${userEmail}`
  ...
  function showMasjidDetails(masjid) {
    const masjidName = document.getElementById("masjid-name");
    const masjidAddress = document.getElementById("masjid-address");
    const embedLink = document.getElementById("embed-link");
    const viewMapButton = document.getElementById("view-map");

    masjidName.textContent = masjid.name;
    masjidAddress.textContent = masjid.address;
    embedLink.textContent = masjid.embed_link;

    // Show the details container
    detailsContainer.style.display = "block";

    // Add event listener for the "Lihat Peta" button
    viewMapButton.addEventListener("click", () => {
      window.open(masjid.embed_link, "_blank"); // Open the embed link in a new tab
    });
  }

  // Event listener for closing the details container
